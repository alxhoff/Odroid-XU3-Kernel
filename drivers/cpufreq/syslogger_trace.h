...
#define TRACE_INCLUDE_PATH /home/alxhoff/Work/Optigame/Odroid-XU3-Kernel/drivers/cpufreq
...
TRACE_EVENT(iteration,
	TP_PROTO(struct timespec *raw,
	...
	TP_fast_assign(
		__entry->raw = timespec_to_ns(raw);
		__entry->real = timespec_to_ns(real);
	),

	TP_printk("raw: %llu, real: %llu",
		  __entry->raw, __entry->real)
);

TRACE_EVENT(cpu_info,
	TP_PROTO(
		unsigned char cpu,
		bool online,
		uint64_t system,
...
TRACE_EVENT(cpu_freq,
	...
	TP_printk("cpu: %u, frequency: %u",
		__entry->cpu, __entry->freq)
);
...
TRACE_EVENT(ina231,
	...
	TP_STRUCT__entry(
		__field(unsigned int, a15)
		__field(unsigned int, a7)
	...
	TP_printk("a15: %u, a7: %u, mem: %u, gpu: %u",
...
TRACE_EVENT(mali,
	...
	TP_printk("load: %u, freq: %u",
		  __entry->load, __entry->freq)
);
...
TRACE_EVENT(exynos_temp,
	...
	TP_printk("A15-0: %u, A15-1: %u, A15-2: %u, A15-3: %u, GPU: %u",
		  ...
		  __entry->t3, __entry->t4)
);


TRACE_EVENT(net_stats,
	TP_PROTO(struct rtnl_link_stats64 *stats),

	TP_ARGS(stats),

	TP_STRUCT__entry(
		__field(u64, rx_packets)
		__field(u64, rx_bytes)
		__field(u64, tx_packets)
		__field(u64, tx_bytes)
	),

	TP_fast_assign(
		__entry->rx_packets = stats->rx_packets;
		__entry->rx_bytes = stats->rx_bytes;
		__entry->tx_packets = stats->tx_packets;
		__entry->tx_bytes = stats->tx_bytes;
	),

	TP_printk("rx: %llu (%llu), tx: %llu (%llu)",
		  __entry->rx_packets, __entry->rx_bytes, __entry->tx_packets,
		  __entry->tx_bytes)
);
